			...
			<div className="mx-auto max-w-7xl">
				...
				<section className="mt-8 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
					...
					<div className="mt-4 grid gap-3 md:grid-cols-5">
						<input value={location} onChange={(event) => setLocation(event.target.value)} placeholder="Area or neighborhood" className="rounded-xl border border-gray-300 px-3 py-3 text-sm md:col-span-2" />
						<select value={type} onChange={(event) => setType(event.target.value)} className="rounded-xl border border-gray-300 px-3 py-3 text-sm">{PROPERTY_TYPES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select>
						<select value={bedrooms} onChange={(event) => setBedrooms(event.target.value)} className="rounded-xl border border-gray-300 px-3 py-3 text-sm"><option value="any">Any bedrooms</option><option value="1">1+ bedroom</option><option value="2">2+ bedrooms</option><option value="3">3+ bedrooms</option><option value="4+">4+ bedrooms</option></select>
						<input type="number" value={maxBudget} onChange={(event) => setMaxBudget(event.target.value)} placeholder={mode === 'rent' ? 'Max monthly budget' : 'Max purchase price'} className="rounded-xl border border-gray-300 px-3 py-3 text-sm" />
					</div>
